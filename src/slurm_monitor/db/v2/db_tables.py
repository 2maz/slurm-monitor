...
def ensure_non_negative(*column_names) -> list[CheckConstraint]:
    return [CheckConstraint(f"{x} >= 0", name=f"{x}_is_not_negative") for x in column_names]
    ...
    def process_bind_param(self, value, dialect):
        ...
        for key in value:
            if key not in self.allowed:
                raise KeyError(
                    f"{self.__class__}: value contains an invalid key '{key}'. Permitted are {{','.join(allowed)}}"
                )
        return value
...
class Node(TableBase):
    ...
    __table_args__ = {"info": {"comment": "Auxiliary class to permit foreign key constraints on cluster/node"}}
...
class SampleProcessGpu(TableBase):
    ...
    epoch = Column(BigInteger, desc="Bootcycle presentation of node - continuously increasing number", primary_key=True)
    user = Column(String)
    ...
    __table_args__ = (
        ...
        Index(
            f"{ExtraIndexPrefix}sample_process_gpu__cluster_node_job_epoch_pid",
            "cluster",
            "node",
            "job",
            "epoch",
...
class SampleProcess(TableBase):
    ...
    in_container = Column(Boolean, desc="True if deemed part of a container.", nullable=True)
    ...
    __table_args__ = (
        ...
        Index(
            f"{ExtraIndexPrefix}sample_process__cluster_node_job_epoch_pid",
            "cluster",
            "job",
            "epoch",
            "node",
...
class SampleDisk(TableBase):
    ...
    name = Column(String, primary_key=True, desc="Disk's local name. This must never be empty (and unique per node)")
...
class SampleSlurmJob(TableBase):
    ...
    gres_detail = Column(
        ARRAY(String), nullable=True, desc="Deprecated: use requested_resource and allocated_resources"
    )
...
class SampleSlurmJobAcc(TableBase):
    ...
    __table_args__ = (
        *ensure_non_negative(
        ...
        Index(
            f"{ExtraIndexPrefix}sample_slurm_job_acc__cluster_job_id_job_step_time",
            "cluster",
            "job_id",
            "job_step",
            time.desc(),
