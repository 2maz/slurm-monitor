    ...
    def validate(self, tables: MetaData, show: bool = False):
        ...
        if show:
            ...
            for spec_object, fulfillment in covered_spec.items():
                ...
                for implemented_columns in fulfillment["implemented"].values():
                ...
                if missing_columns:
                    print(
                        f"     {ljust_spec_object} INCOMPLETE - missing implementation of: {','.join(missing_columns)}"
                    )
                else:
                    print(
                ...
                for column in tables[table].columns:
                    if column.name in implemented_columns:
                        try:
                            ...
                            # items here
                            logger.debug(f"No column '{e}' in spec object '{spec_object_name}' - current table {table}")
