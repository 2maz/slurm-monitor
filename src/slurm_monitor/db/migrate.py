...
def migrate(
    ...
    for node in tqdm(v0_nodes, desc="migrating nodes"):
        ...
        for uuid in uuids:
            uuid_gpu_status = v0_db.fetch_all(v0.db.GPUStatus, v0.db.GPUStatus.uuid == uuid)
            gpu_status = uuid_gpu_status[0]

            v1_db.insert_or_update(v1_db.GPUs(
    ...
    for status in tqdm(gpu_status, desc="gpu_status"):
        v1_db.insert_or_update(v1_db.GPUStatus(
            uuid=status.uuid,
...
def ensure_uri(db_uri) -> str:
    ...
    if not path.exists():
        raise RuntimeError("Failed to identify db type, and interpreting as path does not work either")
...
def cli_run():
    ...
    parser.add_argument("--from-db-uri", type=str, help="Source database (sqlite)", required=True)
    parser.add_argument("--to-db-uri", type=str, help="Target database (postgres or sqlite)", required=True)
