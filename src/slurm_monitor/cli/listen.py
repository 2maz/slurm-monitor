...
class ListenParser(BaseParser):
    ...
    def publish_status(self, output: MessageSubscriber.Output) -> MessageSubscriber.Control:
        ...
        try:
            logger.debug(
                f"Listen.publish_status (to listen-ui) - start (from {cluster}) (hwm: {self.socket.get_hwm()})"
            )
            data = dict(output)
            self.socket.send_json(data, default=str, flags=zmq.NOBLOCK)
            self.message_tx_count += 1
            logger.debug(
                f"Listen.publish_status (to listen-ui): complete (message_tx_count={self.message_tx_count} {cluster=})"
            )
        ...
        try:
            logger.debug(
                f"Listen.publish_status (check control cmds from listen-ui) recv_multipart: start (from {cluster})"
            )
...
class ListenUiParser(BaseParser):
    ...
    def __init__(self, parser: ArgumentParser):
        ...
        parser.add_argument("--ui-host", type=str, default="*", help="Set the ui host, default is *")
    ...
    def execute(self, args):
        ...
        def update():
            ...
            try:
                ...
                self.message_rx_count += 1
                logger.info(
                    f"ListenUiParser.recv_multipart: complete (message_rx_count={self.message_rx_count} {dealer_id=})"
                )
