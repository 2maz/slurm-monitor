...
class MCPParser(BaseParser):
    def __init__(self, parser: ArgumentParser):
        ...
        parser.description = (
            "Start a Model Context Protocol (MCP) server that proxies "
        ...
        parser.add_argument(
            ...
            help=f"Base URL of the running RESTAPI to proxy, default is http://localhost:{app_settings.port}/api/v2",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Bearer token to authenticate against the RESTAPI, required only if the RESTAPI has OAuth enabled",
        )
