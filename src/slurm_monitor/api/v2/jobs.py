...
@api_router.get(
    "/cluster/{cluster}/jobs/{job_id}/epoch/{epoch}/info",
    summary="Get job information by id and epoch for the given cluster",
    tags=["job"],
    response_model=JobResponse,
)
...
async def query_jobs_pages(
    ...
    return fastapi_pagination.paginate(
        jobs, params=fastapi_pagination.Params(page=page, size=page_size)
    )
...
async def job_report(
    ...
    return await dbi.get_job_report(cluster=cluster, job_id=job_id, time_in_s=time_in_s)
