...
class Nvidia(GPU):
    ...
    def detect(cls) -> GPUInfo:
        ...
        result = subprocess.run(
            f"nvidia-smi {ids} --query-gpu=gpu_name,memory.total --format=noheader,csv,nounits",
            shell=True,
    ...
    def get_processes(self) -> list[GPUProcessStatus]:
        ...
        response = Command.run(
            f"{self.query_cmd} --query-compute-apps=gpu_uuid,pid,name,used_gpu_memory --format=csv,nounits",
        )
        df = pd.read_csv(StringIO(response.strip()))
