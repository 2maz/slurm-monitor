...
def test_MessageSubscriber_extract_offset_bounds(txt, expected_topic, expected_lower_bound, expected_upper_bound):
...
async def test_MessageSubscriber_sonar_examples(
    ...
    task = asyncio.create_task(
        message_subscriber.consume_topic(
    ...
    for cluster_name, nodes in expected_clusters.items():
        with db.make_session() as session:
            ...
            assert sorted(received_nodes) == sorted(expected_nodes), (
                f"Expected nodes {expected_nodes} in cluster_attributes, but got {received_nodes=}"
            )
            assert sorted(received_partitions) == sorted(expected_partitions), (
                f"Expected partitions {expected_partitions} in cluster_attributes, but got {received_partitions=}"
            )
