...
async def test_DBJsonImporter_sonar_examples(
    ...
    with db.make_session() as session:
        ...
        for expected_cluster, expected_nodes in expected_clusters.items():
            assert clusters[expected_cluster] == expected_nodes, (
                f"Expected {expected_cluster} with nodes {expected_nodes} in cluster_attributes, but got {clusters=}"
            )
            # existing clusters of test db plus newly added ons
            assert len(expected_clusters) + db_config.number_of_clusters == len(clusters)

            if has_cluster:
                nodes_with_states = session.execute(
                    sqlalchemy.text(f"SELECT node from node_state where cluster = '{expected_cluster}'")
                ).all()
                assert expected_nodes == set([x[0] for x in nodes_with_states]), (
                    "Node should be in cluster_attributes and node_state"
                )
        ...
        for uuid in in_msg_uuids:
            ...
            results = session.execute(
                sqlalchemy.text(f"SELECT memory from sysinfo_gpu_card WHERE uuid = '{uuid}'")
            ).all()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            ["0+cluster.ex3.simula.no.json", "0+job-with-unknown-partition.ex3.simula.no.json"],
            {
                "ex3.simula.no": {
                    "nodes": ["g001", "g002", "h001", "n004"],
                    "partitions": ["unknown-partition-0", "dgx2q", "habanaq", "hgx2q", "mi100q"],
                }
            },
        ],
    ],
