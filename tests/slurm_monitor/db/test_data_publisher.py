...
def controller(nodename, monkeypatch):
    def mock_controller__init__(
        self, collector: DataCollector, bootstrap_servers: str, shutdown_event: asyncio.Event, **kwargs
    ):
    ...
    return Controller(
        collector=collector, bootstrap_servers="localhost:10000", shutdown_event=shutdown_event, listen_interval_in_s=5
    )
...
async def test_collector_collect(controller, mock_slurm_command_hint, monkeypatch):
    ...
    collector_task = asyncio.create_task(collector.collect(shutdown_event, publish_fn), name="test_collector_collect")
...
async def test_collector_collect_max_samples(controller, mock_slurm_command_hint, monkeypatch):
    ...
    collector_task = asyncio.create_task(
        collector.collect(shutdown_event, publish_fn, max_samples=3), name="test_collector_collect_max_samples"
    )
    future = asyncio.gather(collector_task)
