...
from slurm_monitor.db.v1.db_tables import GPUIdList, GPUs, GPUStatus, ProcessStatus
...
def test_GPUStatus_merge():
    ...
    status_a = GPUStatus(
        uuid=uuid, temperature_gpu=5, power_draw=10, utilization_gpu=20, utilization_memory=30, timestamp=now
    )

    status_b = GPUStatus(
        uuid=uuid, temperature_gpu=10, power_draw=20, utilization_gpu=30, utilization_memory=40, pstate=1, timestamp=now
    )
