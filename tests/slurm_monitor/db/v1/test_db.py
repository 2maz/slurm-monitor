...
from slurm_monitor.db.v1.db_tables import GPUStatus, JobStatus, ProcessStatus, TableBase
...
async def test_gpu_status(test_db, number_of_nodes, number_of_gpus, number_of_samples):
    for i in range(0, number_of_nodes):
        ...
        resolution_in_s = 10
        gpu_status = await test_db.get_gpu_status(
            node=nodename, resolution_in_s=resolution_in_s
        )
...
async def test_memory_status(test_db, number_of_nodes, number_of_samples):
    for i in range(0, number_of_nodes):
        ...
        status = await test_db.get_memory_status(
            node=nodename, resolution_in_s=resolution_in_s
        )
        assert len(status) >= (number_of_samples / resolution_in_s)

        start_epoch = (
            status[0].timestamp.replace(tzinfo=dt.timezone.utc).timestamp() - 30
        )
        end_epoch = (
            status[-1].timestamp.replace(tzinfo=dt.timezone.utc).timestamp() + 100
        )
        ...
        start_epoch_date = dt.datetime.fromtimestamp(
            start_epoch, dt.timezone.utc
        ).replace(tzinfo=None)
        end_epoch_date = dt.datetime.fromtimestamp(end_epoch, dt.timezone.utc).replace(
            tzinfo=None
        )
        for x in status:
...
@pytest.mark.asyncio(loop_scope="module")
async def test_memory_status_timeseries_list(
    test_db, number_of_nodes, number_of_samples
):
    nodes = [f"node-{i}" for i in range(0, number_of_nodes)]
    resolution_in_s = 10

    status = await test_db.get_memory_status_timeseries_list(
        nodes=nodes, resolution_in_s=resolution_in_s
    )
...
@pytest.mark.asyncio(loop_scope="module")
async def test_gpu_status_timeseries_list(
    test_db, number_of_nodes, number_of_gpus, number_of_samples
):
    nodes = [f"node-{i}" for i in range(0, number_of_nodes)]
    resolution_in_s = 10
