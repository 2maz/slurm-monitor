...
async def test_fetch_first(test_db, number_of_nodes, number_of_gpus):
    results = await test_db.fetch_first_async(
        GPUStatus, order_by=GPUStatus.timestamp.desc()
    )
    assert (
        results.timestamp
        == test_db.fetch_first(GPUStatus, order_by=GPUStatus.timestamp.desc()).timestamp
    )
