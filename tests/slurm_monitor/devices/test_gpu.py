...
@pytest.mark.parametrize(
    "gpu_type,response,model,count,memory_total",
    [
        ["nvidia", ["Tesla K20m, 5062", "Tesla K20m, 5062"], "Tesla K20m", 2, 5062],
        [
            "rocm",
            [
...
def test_GPUInfo_detect_x_smi(
    gpu_type, response, model, count, memory_total, monkeypatch, mocker
):
...
def test_GPU_transform(
    cls: GPU, response: list[str] | dict[str, any], gpu_count: int, monkeypatch
):
    if cls == XPU:
        # XPU Specifics - since memory total can only be identified using 'discovery'
