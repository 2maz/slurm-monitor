...
@pytest.fixture(scope="module")
def test_db(
    test_db_uri, number_of_nodes, number_of_cpus, number_of_gpus, number_of_samples
) -> SlurmMonitorDB:
...
GPU_RESPONSES = {
    "nvidia": {
        "discover": ["Tesla V100-SXM3-32GB, 32768", "Tesla V100-SXM3-32GB, 32768"],
        "probe": [
    ...
    "rocm": {
        ...
        "showpids": ["name, value", '"PID3903703", "python, 2, 12333932544, 0, 11"'],
        "expect": {
            "gpu_count": 2,
...
@pytest.fixture(scope="module")
def test_db_v2(timescaledb, db_config) -> db_v2.db.ClusterDB:
    for i in range(0, 3):
        try:
...
def test_db_v2__function_scope(timescaledb, db_config) -> db_v2.db.ClusterDB:
    for i in range(0, 3):
        try:
            db_test = db_v2_testing.create_test_db(timescaledb, db_config)
            time.sleep(2)
